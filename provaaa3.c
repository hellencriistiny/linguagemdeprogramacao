...
void criarMatriz(int m, int n, char matriz[MAX_LINHAS][MAX_COLUNAS]) {
    ...
    for (i = 0; i < m; i++) {
        for (j = 0; j < n; j++) {
            printf("Informe o elemento da posição (%d, %d): ", i + 1, j + 1);
            scanf("%s", matriz[i][j]);
        }
    }
}
...
int main() {
    ...
    // Imprimir a matriz
    printf("Matriz inserida pelo usuário:\n");
    ...
    printf("A palavra '%s' aparece %d vezes na direção horizontal e %d vezes na direção vertical.\n", palavra, countHorizontal, countVertical);
